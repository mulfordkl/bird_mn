...
In the last post about eBird data I went through how I acquired, cleaned, and explored the dataset. In this project, I am exploring how the eBird data can be used to model the encounter rate of the White Breasted Nuthatch in Minnesota. As with the last post, I have been heavily referencing the eBird best practices eBook. The first task is to load the large number of libraries we will be using.  
...
And to set the paths of our data sets.
...
What I can see here is that our model consistently underestimates the encounter rate but does a better job near the upper limit of predicted encounter rate. We can now calculate some model performance metrics on the held-out test set.
...
pdp::partial(rf, pred.var = "pland_12_cropland", plot = TRUE)  # Figure 2
