...
# load ebird data
ebird <- read_csv(here("data", "ebd_whbrnuthatch_yearround_mn_zf.csv"))
# get list of tiles required to cover this bcr
tiles <- getTile(extent = list(xmin = -98, xmax = -89, ymax = 49.5, ymin = 43))
```
...
#mn <- read_sf("data/gis-data.gpkg", "ne_state_lines") 
#%>% 
#  filter(bcr_code == 27) %>% 
...
forest_cover <- pland_coords %>% 
  ...
  st_transform(crs = projection(r)) %>% 
  # rasterize points
  rasterize(r, field = "pland_12_cropland") %>% 
